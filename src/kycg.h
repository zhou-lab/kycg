 ...
 * binary, so that CLI entry symbol would otherwise be doubly defined. test /
 * annotate have no libyame counterpart. (File inspection is `yame info` now, so
 * kycg no longer carries its own info subcommand.) */
